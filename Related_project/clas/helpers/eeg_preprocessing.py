...
Moved from d02_preprocessing.py to keep the codebase organized
"""
...
from clas.helpers.mymath import morlet_transform

#todo: add CSD reference
def apply_reference(eeg_data: pd.DataFrame, reference_channels, exclude_channels=None) -> pd.DataFrame:
    """
    Apply channel-based reference
    
    Args:
        eeg_data: EEG DataFrame with channels as columns
        reference_channels: 'average', list of channel indices, or single channel #0-indexed
        exclude_channels: List of channel indices to exclude from average reference computation #1-indexed
    
    Returns:
        Re-referenced EEG data
    """
    ref_data = eeg_data.copy()

    #todo: add CSD reference
    
    if reference_channels == 'average':
        # Average reference across all channels, excluding bad channels
        if exclude_channels is not None:
            # Get clean channels for average reference computation
            clean_channels = []
            for i, col in enumerate(ref_data.columns):
                # Check if channel should be excluded 
                if i not in exclude_channels:
                    clean_channels.append(i)
            
            if clean_channels:
                # Compute average reference using only clean channels
                ref_mean = ref_data.iloc[:, clean_channels].values.mean(axis=1, keepdims=True)
            else:
                # Fallback: use all channels if no clean channels found
                ref_mean = ref_data.values.mean(axis=1, keepdims=True)
        else:
            # No exclusions - use all channels
            ref_mean = ref_data.values.mean(axis=1, keepdims=True)
    else:
        # Channel-based reference - use iloc for integer indexing
        if isinstance(reference_channels, list):
            # Convert channel numbers to column indices
            channel_indices = []
            for ch in reference_channels:
                if ch in ref_data.columns:
                    channel_indices.append(ref_data.columns.get_loc(ch))
                else:
                    # Fallback: try to find the channel by position
                    channel_indices.append(ch)
            
            # Use iloc for integer-based indexing
            ref_mean = ref_data.iloc[:, channel_indices].values.mean(axis=1, keepdims=True)
        else:
            # Single channel reference
            if reference_channels in ref_data.columns:
                ch_idx = ref_data.columns.get_loc(reference_channels)
                ref_mean = ref_data.iloc[:, ch_idx].values.reshape(-1, 1)
            else:
                # Fallback: try direct indexing
                ref_mean = ref_data[reference_channels].values.reshape(-1, 1)
    
    ref_data = ref_data - ref_mean
    return ref_data


def exclude_bad_channels(data, exclude_channels):
    """Simple helper to exclude bad channels from DataFrame"""
    ...
    # Drop bad channels (0-indexed column names)
    bad_cols = [col for col in exclude_channels if col in df.columns]
...
#todo: use MNE resampling instead
def apply_resample(eeg_data: pd.DataFrame, resample_params: Dict[str, Any]) -> pd.DataFrame:
    """
    Resample EEG data to target sampling rate using groupby approach
    
    Args:
        eeg_data: Input EEG DataFrame
        resample_params: Dictionary containing 'original_sf', 'target_sf' keys
        
    Returns:
        Resampled EEG DataFrame
    """
    original_sf = resample_params['original_sf']
    target_sf = resample_params['target_sf']
    
    if target_sf is None or target_sf == original_sf:
        return eeg_data
    
    # Calculate resampling factor
    resample_factor = int(original_sf / target_sf)
    
    if resample_factor <= 1:
        # No downsampling needed 
        return eeg_data
    
    # Store original start index for proper reindexing
    start_index = eeg_data.index[0] if len(eeg_data.index) > 0 else 0
    
    # Resample using groupby approach (similar to notebook)
    # Use the original index for the grouping operation
    resampled_data = eeg_data.groupby(np.arange(len(eeg_data)) // resample_factor).mean()
    
    # Reindex to maintain proper time indices
    resampled_data.index = np.int64(start_index + np.arange(len(resampled_data)) * resample_factor)
    
    return resampled_data


def apply_bandpass(eeg_data: pd.DataFrame, bandpass_params: Dict[str, Any]) -> pd.DataFrame:
    """Apply bandpass filter"""
    min_freq = bandpass_params['min_freq']
    max_freq = bandpass_params['max_freq']
    sf = bandpass_params.get('sf', 125)
    ...
    for col in filtered_data.columns:
        filtered_data[col] = sosfiltfilt(sos, filtered_data[col])
    
    return filtered_data

def apply_highpass(eeg_data: pd.DataFrame, highpass_params: Dict[str, Any]) -> pd.DataFrame:
    """Apply highpass filter"""
    highpass_freq = highpass_params['highpass_freq']
    sf = highpass_params.get('sf', 125)
...
def apply_wavelet(eeg_data: pd.DataFrame, wavelet_params: Dict[str, Any]) -> np.ndarray:
    """Apply Morlet wavelet transform"""
    freq = wavelet_params['freq']
    cycles = wavelet_params['cycles']
    # Prefer explicit 'sf' (sampling rate). Fallback to legacy 'resample_rate' for backward compatibility.
    sf = wavelet_params.get('sf', wavelet_params.get('resample_rate', 25))
    
    # Convert to numpy and apply wavelet
    wdata = morlet_transform(eeg_data.values, sf, freq, cycles, n_jobs=1)
    return wdata


def apply_hilbert(eeg_data: pd.DataFrame, hilbert_params: Dict[str, Any]) -> np.ndarray:
    """Apply Hilbert transform"""
    sf = hilbert_params.get('sf', 125)
    
    # Apply Hilbert transform
    hdata = hilbert(eeg_data.values, axis=0)
    return hdata


def process_eeg_data(eeg_data: pd.DataFrame, preprocessing_steps: list) -> Union[pd.DataFrame, np.ndarray]:
    """
    Apply multiple preprocessing steps to EEG data
    
    Args:
        eeg_data: Raw EEG data
        preprocessing_steps: List of (function, params) tuples
    ...
    for step_func, step_params in preprocessing_steps:
        if step_func == 'resample':
            current_data = apply_resample(current_data, step_params)
        elif step_func == 'reference':
            current_data = apply_reference(current_data, step_params)
        elif step_func == 'bandpass':
            current_data = apply_bandpass(current_data, step_params)
        elif step_func == 'wavelet':
            current_data = apply_wavelet(current_data, step_params)
        elif step_func == 'hilbert':
            current_data = apply_hilbert(current_data, step_params)
        else:
            raise ValueError(f"Unknown preprocessing step: {step_func}")
